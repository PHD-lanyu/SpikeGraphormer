...
def load_nc_dataset(args,datapath=None,no_feat_norm=False):
    ...
    if dataname == 'deezer-europe':
        ...
        dataset = load_planetoid_dataset(dataname, no_feat_norm)
    elif dataname in ('film','texas', 'wisconsin','cornell'):
        dataset = load_geom_gcn_dataset(dataname)
    elif dataname in ('chameleon', 'squirrel'):
        dataset=load_wiki_new(dataname,no_feat_norm)
        # dataset = load_wikipedia(dataname,args.no_feat_norm)
    elif dataname == 'mini':
        dataset = load_mini_imagenet(args.data_dir)
    elif dataname == '20news':
        dataset = load_20news(args.data_dir)
    else:
